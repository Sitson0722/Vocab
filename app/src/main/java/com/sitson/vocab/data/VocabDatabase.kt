...
interface VocabDao {
    ...
    @Query("UPDATE ProgressEntity SET stabilityDays=:stability, difficulty=:difficulty, consecutiveSuccesses=:successes, lapses=:lapses, dueAt=:dueAt, attempts=attempts+1 WHERE wordId=:wordId AND dimension=:dimension")
    suspend fun updateProgress(wordId: Long, dimension: String, stability: Double, difficulty: Double, successes: Int, lapses: Int, dueAt: Long)
    ...
    @Query("SELECT w.id,w.term,w.definition,w.phrase,w.example,p.dimension,p.dueAt,p.attempts FROM WordSenseEntity w JOIN ProgressEntity p ON p.wordId=w.id WHERE p.attempts>0 AND p.dueAt<=:now ORDER BY p.dueAt, p.difficulty DESC LIMIT :limit")
    suspend fun due(now: Long, limit: Int): List<StudyItem>
    @Query("SELECT w.id,w.term,w.definition,w.phrase,w.example,p.dimension,p.dueAt,p.attempts FROM WordSenseEntity w JOIN ProgressEntity p ON p.wordId=w.id WHERE p.attempts=0 ORDER BY w.createdAt, CASE p.dimension WHEN 'COMPREHENSION' THEN 0 ELSE 1 END LIMIT :limit")
...
@Database(entities = [WordSenseEntity::class, ProgressEntity::class, AttemptEntity::class], version = 1, exportSchema = true)
abstract class VocabDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        fun get(context: Context): VocabDatabase = instance ?: synchronized(this) {
            instance ?: Room.databaseBuilder(context, VocabDatabase::class.java, "vocab.db").build().also { instance = it }
        }
    }
}
